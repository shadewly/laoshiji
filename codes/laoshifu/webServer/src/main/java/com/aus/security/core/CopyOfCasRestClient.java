...
public class CopyOfCasRestClient {
	...
	private static String getTicketGrantingTicket(final String server,
		...
		try {
			...
			final String responseEntityStr = response.getHeaders("Location")[0].getValue();
			info("TGT=" + response);
			switch (response.getStatusLine().getStatusCode()) {
			case 201: {
//				final Matcher matcher = Pattern.compile(
//						".*/(.*).*").matcher(responseEntityStr);
//				".*action=\".*/(.*?)\".*"

				if (responseEntityStr.contains("/TGT")){
				
					
					return responseEntityStr.substring(responseEntityStr.indexOf("/TGT")+1);
				}
